...
class SchedGraphBuilder {
    ...
    makeGanttDiagram() {
        if (!recipieBuilder.circle) {
            ...
            let offset = Math.ceil((recipieBuilder.longestPathTime + 1) / (document.getElementById("gantt-diagram").parentElement.offsetWidth / 40));
            console.log(offset);
            ...
            for (var j = 0; j < this.gantt.length; j++) {
                for (var i = 0; i < this.gantt[j].tasks.length; i++) {
                    ...
                    width /= offset;
                    y = y_unit * j;
                    x = this.gantt[j].tasks[i].start_time * 40;
                    x /= offset;
                    x += 40;
                    color = "#E2E2E2";
            ...
            for (var j = 0; j < recipieBuilder.longestPathTime + 2; j += offset) {
                x += 40;
